...
public class MovieProvider extends ContentProvider {
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch (match) {
            case FAVORITE: {
                ...
                if (_id > 0)
                    returnUri = MovieContract.FavoriteMovie.buildMovieUriWithId(_id);
                else
        ...
        switch (match) {
            case FAVORITE:
                rowsDeleted = db.delete(
                        MovieContract.FavoriteMovie.TABLE_NAME, selection, selectionArgs);
                break;
            default:
                throw new UnsupportedOperationException("Unknown uri: " + uri);
        }
    ...
    public int update(
        ...
        switch (match) {
            case FAVORITE:
                rowsUpdated = db.update(MovieContract.FavoriteMovie.TABLE_NAME, values, selection,
                        selectionArgs);
                break;
            default:
                throw new UnsupportedOperationException("Unknown uri: " + uri);
            ...
            case FAVORITE:
                ...
                try {
                    for (ContentValues value : values) {
                        long _id = db.insert(MovieContract.FavoriteMovie.TABLE_NAME, null, value);
                        if (_id != -1) {
                            returnCount++;
                        }
                    }
